...
def demo():
    ...
    fid = TextGrid.load(test_textgrid)
    for i, tier in enumerate(fid):
        if tier.nameid == "words":
            for st, end, word in tier.simple_transcript:
                print("word {} duration is {}".format(word, float(end)-float(st)))
        if tier.nameid == "phones":
            for st, end, phone in tier.simple_transcript:
                print("phone {} duration is {}".format(phone, float(end)-float(st)))
