...
from espnet2.tts.emoFeats_extract.harmonic import Harmonic
from espnet2.tts.emoFeats_extract.jilter import Jilter
import numpy as np


class ContrlEncoder(torch.nn.Module):
    """contrl encoder

    Args:
        contrl_num(int, optional): Number of controlling features. har, jil, ..
        units(int, optional):
    ...
    def __init__(self,
                 contrl_num: int = 10,
                 units: int = 128):
        assert contrl_num == 10
        har = Harmonic()
        jil = Jilter()
        ...
        contrl_en = torch.nn.Linear(contrl_num, units)

    def forward(self, y: torch.Tensor) -> torch.Tensor:
        # observable
        har_c = self.har.forward(y)
        jil_c = self.jil.forward(y)

        #

        contrl = np.concatenate(har_c, jil_c, ...)
        contrl_token = self.contrl_en(contrl)
        return contrl_token
...
class ReferenceEncoder(torch.nn.Module):
    ...
    Args:
        ...
        conv_kernel_size (int, optional):
            Kernal size of conv layers in the reference encoder.
        conv_stride (int, optional):
            Stride size of conv layers in linethe reference encoder.
        gru_layers (int, optional): The number of GRU layers in the reference encoder.
        gru_units (int, optional): The number of GRU units in the reference encoder.
    ...
    def __init__(
        ...
        assert conv_kernel_size % 2 == 1, line"kernel size must be odd."
        assert (
        ...
        for i in range(conv_layers):
            ...
            convs += [
                ...
                torch.nn.ReLU(inplace=Trueline),
            ]
